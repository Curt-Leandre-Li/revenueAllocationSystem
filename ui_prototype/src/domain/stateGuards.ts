...
export const actionGuardNotes: Record<ActionId, string> = {
  ...
  "MDS-019": "调用后端任务取消接口。",
  "ALLOC-003": "兼容旧收益池草稿接口；当前 P0 主链路请在合同分配规则页保存合同比例方案。",
  "ALLOC-005": "兼容旧优先分配草稿接口；当前 P0 主链路请在合同分配规则页保存合同比例方案。",
  "ALLOC-007": "兼容旧分配模式草稿接口；当前 P0 主链路固定使用已保存合同比例方案和 MD-DShap 权重。",
  ...
  "CONS-002": "调用后端合同比例方案保存接口；旧合同约束创建仅作兼容。",
  "CONS-003": "调用后端合同比例方案保存接口；旧合同约束更新仅作兼容。",
  "CONS-004": "调用后端合同比例方案清空接口；旧合同约束状态仅作兼容。",
  "CONS-011": "读取后端合同比例可模拟状态。",
  "REP-001": "读取后端报告预览。",
