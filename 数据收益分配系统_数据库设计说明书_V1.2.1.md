...
| **项目**     | **内容**                                                                                                             |
|--------------|----------------------------------------------------------------------------------------------------------------------|
| 文档名称     | 《数据收益分配系统 V1.2.1 数据库设计与 ER 关系图》                                                                 |
| 系统名称     | 数据收益分配系统（DVAS，Data Value Allocation System）                                                               |
| 文档版本     | V1.2.1                                                                                                               |
| 编写日期     | 2026 年 6 月 30 日                                                                                                   |
| 设计依据     | 《数据收益分配系统 V1.4.3 需求规格说明书》《数据收益分配系统 V1.3.1 系统详细功能设计文档》、PostgreSQL 参考 DDL 和当前版本运行时对象 |
| 适用范围     | 当前版本本地演示系统与后续权限、PDF、异步任务扩展的数据库设计输入                                                   |
| 更新重点     | 统一左侧导航结构、菜单权限、模块映射、路由路径、ER 图分组和 DDL 种子数据；业务表、算法表、审计快照策略保持不变。 |
...
| V1.1                | 2026-06-25 | 新增逐资源质量评估表、关系、索引、按钮副作用和导出字段映射。 | 已提交 |
| V1.2                | 2026-06-28 | 按 PostgreSQL DDL、`JsonFileRepository` 运行时 state、`persistence_mapping.py` 和合同比例主流程标注已落表、JSON 运行时对象、PostgreSQL 预留与枚举映射。 | 已提交 |
| V1.2.1              | 2026-06-30 | 修正文档元数据、导航映射顺序、JSON runtime 与 PostgreSQL DDL 边界说明。 | 终稿 |

## 0. 当前版本数据边界（2026-06-30）

本节说明当前版本运行时存储、PostgreSQL 参考 DDL 和后续生产化映射的边界。若后续表格保留候选表或早期字段，与本节冲突时，以本节为准。

### 0.1 当前存储模型

| 层级 | 当前版本边界 | 文档口径 |
|------|----------|----------|
| 当前版本运行时 | 后端默认使用 `JsonFileRepository`，写入 `backend/runtime/dvas_state.json`。 | 当前业务写入以 JSON state 为准。 |
...
| Postgres read model | `PostgresReadService` 只提供健康检查、项目、状态、审计、报告等只读查询。 | 不承担当前版本主写路径。 |
| 枚举映射 | `persistence_mapping.py` 将 runtime 枚举映射到 SQL 枚举。 | 文档列出映射，区分 runtime 值域与 SQL 字段值域。 |
...
以下对象当前存在于 JSON runtime state，但没有出现在 `db/dvas_p0_01_schema.sql` 中，应按运行时对象或后续生产化预留对象描述：`contract_ratio_plans`、`contract_ratio_items`、`quality_resource_assessments`、`quality_resource_score_details`、`report_manifests`、`sessions`、`async_jobs`、`business_drafts`。
...
`contract_constraint` / `constraint_apply_trace` 是旧约束兼容模型，只影响旧 `allocation/simulation/run` 或 `allocation-scenarios` 路径。项目级合同比例主流程通常不生成普通 `constraint_apply_trace`，合同分配规则页以 `contract_ratio_plans` / `contract_ratio_items` 为主。
...
当前收益分配计算下的业务显示顺序是 MD-DShap 计算管理、合同分配规则、收益分配模拟；`db/dvas_p0_02_seed.sql` 中保留 MD-DShap 计算管理、收益分配模拟、合同约束管理的初始化记录。本文档按当前业务导航和 PostgreSQL 参考 DDL 双口径记录，不在本文件中改写数据库脚本。
...
下图按更新后的六类一级导航分组展示主要表关系。ER 图图片、SVG 和 Mermaid 源码随本文档交付资产一起保存。
...
| pre_constraint_amount        | numeric(18,2) | Y        |        | 当前合同比例路径下用于记录模拟前展示金额。 |
| post_constraint_amount       | numeric(18,2) | Y        |        | 当前合同比例路径下用于记录模拟后参考金额。 |
...
| 质量评估管理         | 资源级质量导出边界             | 当前版本无独立资源级质量导出接口；资源级质量结果可在质量页面查看，也可作为综合报告章节展示                       | 无独立导出成功日志         | 后续扩展能力              |
...
| 用户与权限管理（后续扩展） | USER-002/005/007/010/011/009 用户、密码与权限 | user_account, role, permission, user_role, role_permission                                                    | audit_log，不记录明文密码或 password_hash | 后续扩展 RBAC；停用、重置、本人改密和一次性密码展示 |
| 审计日志管理         | AUD-002/006/007 查询/导出日志  | audit_log, snapshot_store, report_record                                                                      | audit_log                 | 只读追溯                  |

# 12. 报告导出字段与数据来源映射

当前版本导出由 `ReportService` 生成，每次写 `report_record`、`export_file`、报告快照和 checksum。下表区分当前版本会生成的文件、条件生成文件和后续扩展能力。

| **文件名** | **数据来源** | **格式** | **字段/章节清单** | **当前阶段** |
|------------|--------------|----------|-------------------|--------------|
| allocation_summary.md | project、data_package、data_resource、party、quality_assessment、shuyuan_metering、utility_record、md_dshap_task/result、allocation_scenario/result、runtime contract_ratio objects | Markdown | 项目概览、模拟参考声明、数据包与资源摘要、质量摘要、数元摘要、贡献效用摘要、MD-DShap 权重摘要、合同比例摘要、旧约束兼容说明、分配结果、审计说明 | 当前版本 |
...
| quality_assessment_summary.csv | quality_assessment | CSV | assessment_id, quality_score, quality_level, quality_factor, output_snapshot_id；仅当存在质量评估时生成 | 当前版本条件生成 |
| shuyuan_metering_detail.csv | shuyuan_metering、shuyuan_metering_detail | CSV | metering_id, resource_id, party_id, party_name, valid_units, metering_amount；仅当存在计量明细时生成 | 当前版本条件生成 |
| md_dshap_weights.csv | md_dshap_task、md_dshap_result | CSV | task_id, party_id, party_name, participant_weight, normalized_weight, baseline_weight, weight_diff；仅当存在权重结果时生成 | 当前版本条件生成 |
| constraint_apply_trace.csv | constraint_apply_trace | CSV | allocation_id, constraint_id, party_id, before_amount, after_amount, adjustment_amount, reason, version_no；仅在旧约束兼容路径有普通约束 trace 时生成，不属于合同比例主流程输出 | 兼容路径条件生成 |
| allocation_result.json | allocation_scenario/result、runtime contract_ratio objects、quality/shuyuan/utility/md_dshap 结果、snapshot_store | JSON | project_id, allocation_id, contract_ratio_plan, contract_ratio_items, priority_allocation, data_provider_revenue_pool, weight_source, results, constraints, snapshot refs, quality_result, metering_result, utility_result, md_dshap_result, allocation_summary, assumptions, disclaimer | 当前版本 |
| md_dshap_audit_report.md | md_dshap_task/result/marginal_trace、utility_record/trace、snapshot refs | Markdown | algorithm_mode, algorithm_version, task_set, parameters, utility source, participant set, weights, marginal trace summary, snapshot refs, checksum note | 当前版本 |
| md_dshap_audit_report.json | md_dshap_task/result/marginal_trace、utility_record/trace、snapshot refs | JSON | 与 Markdown 审计说明同源的结构化 payload | 当前版本 |
| audit_log.jsonl | audit_log、report_record、export_file | JSONL | record_type, log/report/export fields, status, failure_reason, checksum | 当前版本 |
| p1_simulation_reference_report.pdf | Markdown 报告内容、report_record、export_file、report_manifest runtime object | PDF | 模拟参考报告摘要、英文和中文免责声明、manifest | 后续扩展版本 |
| resource_quality_assessment_result.json / resource_quality_assessment.csv | runtime quality resource objects | JSON/CSV | 当前版本没有独立资源质量导出接口；资源级结果可在质量页面查看并进入综合报告章节 | 后续扩展能力 |
| allocation_confirmation_statement.md | allocation_scenario/result | Markdown | 当前版本没有确认书专用导出接口；Markdown 报告不作为收益分配确认书 | 后续扩展能力 |
| assumptions.json | snapshot_store/system_parameter | JSON | 当前版本没有独立 assumptions 导出接口；assumptions 在 `allocation_result.json` 内嵌 | 后续扩展能力 |

# 13. 快照、版本与审计策略

| **策略项**   | **数据库实现**                                                           | **规则**                                                                              |
|--------------|--------------------------------------------------------------------------|---------------------------------------------------------------------------------------|
| 输入快照     | input_snapshot + snapshot_store(snapshot_type=INPUT)                     | 演示数据初始化、JSON 上传时生成；后续扩展版本复制新版本时也生成，后续计算引用 checksum。 |
...
| 合同约束   | contract_constraint + constraint_apply_trace | constraint_type, constraint_value, before_amount, after_amount, reason                    | 旧约束兼容路径保留，不作为当前合同分配规则页主流程。               |
...
本次数据库设计更新区分运行时 JSON state、PostgreSQL 验收 DDL 和后续正式库预留对象。系统首页、数据管理、数元贡献度计量、收益分配计算、报告生成与导出、系统管理六类入口已映射到 nav_menu、permission、module_code、route_path、audit_log.menu_code 和 ER 图分组。业务数据链路仍是“数据接入 -\> 数据资源/参与方 -\> 质量评估 -\> 数元计量 -\> 贡献效用 -\> MD-DShap -\> 合同分配规则 -\> 收益分配 -\> 报告导出 -\> 审计追溯”；当前合同页以运行时 `contract_ratio_plans` / `contract_ratio_items` 为主，旧 `contract_constraint` / `constraint_apply_trace` 仅作为兼容路径。
