...
| allocation_scenario          | 收益分配场景      | allocation_id        | project_id、weight_task_id                                         | total_revenue、priority_allocation_amount、data_provider_revenue_pool、allocation_mode、status | 总收益、非数据源合同优先合计和数据源主体收益池。 |
| allocation_priority_item     | 合同优先分配项    | item_id              | allocation_id、party_id                                            | priority_amount/ratio、cap_amount、basis_text    | 非数据源主体合同优先分配项，先于数据源收益池扣除。 |
| contract_constraint          | 合同约束表        | constraint_id        | project_id、party_id                                               | constraint_type、constraint_value、priority      | 合同约束管理主表。                             |
| allocation_result            | 分配结果表        | result_id            | allocation_id、party_id                                            | subject_track、raw_weight、pre/post amount       | 数据源主体按 DATA_PROVIDER_POOL 轨道保存；非数据源合同优先金额不伪装为 MD-DShap 结果。 |
...
| priority_allocation_amount | numeric(18,2) | Y        |        | 非数据源主体合同优先分配合计。                                          |
| data_provider_revenue_pool | numeric(18,2) | Y        |        | 扣除合同优先后，留给数据源主体分配的收益池。                            |
...
| party_id        | varchar(64)   | Y        | FK     | 非数据源优先分配主体。 |
| value_type      | varchar(16)   | Y        |        | AMOUNT/RATIO。       |
| priority_amount | numeric(18,2) | N        |        | 请求优先金额。       |
| priority_ratio  | numeric(18,6) | N        |        | 请求优先比例。       |
| cap_amount      | numeric(18,2) | N        | P0/P1 扩展 | 合同优先上限；若当前 schema 未落地，可用 contract_constraint.constraint_type=CAP_AMOUNT / MAX_AMOUNT 表达。 |
...
| project_id                   | varchar(64)   | Y        | FK     | 所属项目。               |
| party_id                     | varchar(64)   | Y        | FK     | 数据源主体。             |
| subject_track                | varchar(40)   | N        | DTO/快照 | DATA_PROVIDER_POOL；如统一展示非数据源最终金额，应使用 CONTRACT_PRIORITY 标记，不写作 MD-DShap 权重分配。 |
...
| 收益分配   | allocation_scenario + allocation_result      | total_revenue, priority_allocation_amount, data_provider_revenue_pool, allocation_mode, pre/post amount | 先扣非数据源主体合同优先并受上限约束，再对数据源主体收益池分配。   |
| 合同约束   | contract_constraint + constraint_apply_trace | constraint_type, constraint_value, before_amount, after_amount, reason                    | 可用 CAP_AMOUNT / MAX_AMOUNT 表达非数据源主体合同上限；约束前后金额并列保存，支持方案对比。 |
...
约束条件：`total_contract_priority_amount <= total_revenue`、`data_provider_revenue_pool >= 0`、`sum(md_dshap_normalized_weight_i) = 1`。
