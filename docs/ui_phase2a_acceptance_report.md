...
| 按钮/表格/抽屉/弹窗是否真实可用 | 通过。系统首页按钮触发路由、确认和模拟记录；风险提示为页面内区块，不设置二级窗口。 | 通过。详情、绑定、导出、切换计算状态均可用。 | 通过。计算、进度、trace、权重、复杂度、导出、审计说明均可用。 |
...
系统首页不设置二级窗口。其他页面不存在未经触发即默认展开的抽屉、弹窗、详情面板或配置窗口；确需二级窗口的页面必须通过一级页面按钮或操作入口打开，并提供关闭或取消路径。

## 系统首页层级检查

`/dashboard` 符合“仅一级页面”要求。默认首屏不存在抽屉、弹窗或详情面板；风险提示作为系统首页内部区块直接展示，不再通过抽屉打开。
...
- `/dashboard`：选择演示数据有业务反馈；进入数据接入、查看报告可跳转；风险提示以内联区块直接展示。
...
未发现系统首页下二级窗口。其他页面未发现无法关闭的弹窗、抽屉或二级窗口，未发现无法返回上一级的交互流程。
...
5. 系统首页不得设置二级窗口；其他页面的二级窗口必须默认关闭，只能由按钮、行操作或明确入口触发。
...
- 每批页面完成后必须重复本报告中的字段泄露、按钮、系统首页无二级窗口、其他页面二级窗口、build、diff check 和截图验收。
