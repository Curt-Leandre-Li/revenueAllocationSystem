...
|---|---|---|
| `数据收益分配系统_V1.3_需求规格说明书_导航结构更新版.md` | 3.4 version boundary, 4.1 system scope, 4.2 non-scope, 4.3-4.4 roles and permission rules, 5.2 data objects, 6.1-6.3 main flow and state machine, 7.1-7.13 button-level requirements, 8.1-8.4 MD-DShap, 9 export fields, 10.2-10.3 exception/audit, 12 acceptance, 13 priority list. | Product source of truth for business semantics, P0/P1 boundary, module inclusion, button semantics, `SYS-004`, `USER-008`, export and audit rules. |
| `数据收益分配系统_系统详细功能设计_V1.1_导航结构更新版.md` | 3.2 module table, 3.3 chain, 3.4 states, 4.2 action types, 5.1 routes, 5.2-5.3 UI rules/components, 6.1 data objects, 7.1-7.14 page/button design, 8.4 MD-DShap cards, 9 exports, 10 service inputs, 11 enums, 12 audit/snapshot, 14 UI constraints, 15 acceptance. | Detailed source for page layout, route paths, action types, overlay expectations, success/failure UI, audit and snapshot requirements. |
| `数据收益分配系统_数据库设计与ER关系图_V1.0_导航结构更新版.md` | 2 navigation-to-database mapping, 3 domain grouping, 4 logical model, 7.1 `nav_menu` / `permission`, 10 enums, 11 button side effects, 12 export data sources, 13 snapshot/audit policy, 14 MD-DShap storage, 15 seed data, 16 P0/P1 DB boundary. | Database source for `menu_code`, `module_code`, `route_path`, main objects, permission actions, status enum, audit fields, `report_id` and `checksum`. |
...
| `ALLOC_SIMULATION` | PRD 4.1 row 120; PRD 7.8; detailed 5.1 and appendix; DB 2 row 58. | ALLOC | `/allocation/simulation` | `NAV_ALLOC_SIMULATION` | `allocation_scenario`, `allocation_priority_item`, `allocation_result` | UPDATE, CALCULATE, CONFIRM, EXPORT | Yes. | Partial: generic page shell/action matrix. | No source drift; renderer not schema-driven. |
| `CONS_MANAGEMENT` | PRD 4.1 row 121; PRD 7.9; detailed 5.1, 7.10, appendix; DB 2 row 59. | CONS | `/allocation/constraints` | `NAV_ALLOC_CONSTRAINT` | `contract_constraint`, `constraint_apply_trace` | CREATE, UPDATE, DELETE_DISABLE, VIEW | Yes. | Partial: generic page shell/action matrix. | No source drift; renderer not schema-driven. |
...
| ALLOC-005 | PRD `ALLOC-005`; detailed appendix `ALLOC-005`. | Configure contract priority allocation for non-data parties. | 配置合同优先分配 | No project state change. | Required | Priority item saved; amount exceeds revenue. | OK. |
| ALLOC-007 | PRD `ALLOC-007`; detailed appendix `ALLOC-007`. | Select allocation mode; MD-DShap weight is default. | 选择分配模式 | No project state change. | Required | Mode enabled; missing weight blocked. | OK. |
| ALLOC-011 | PRD `ALLOC-011`; detailed appendix `ALLOC-011`; DB side effect row `ALLOC-011`. | Run allocation simulation after priority, weights, constraints. | 执行收益分配模拟 | 已计算权重 -> 已分配. | Required | Allocation results; priority/weight failures. | OK. |
...
| CONS-002 | PRD `CONS-002`; detailed appendix `CONS-002`; DB group row `CONS-002/003`. | Create contract constraint. | 新增合同约束 | Constraint enabled. | Required | Constraint created; invalid amount/ratio error. | OK. |
| CONS-003 | PRD `CONS-003`; detailed appendix `CONS-003`; DB group row `CONS-002/003`. | Edit unlocked constraint without rewriting history. | 编辑合同约束 | Constraint version/status changes. | Required | Change saved; exported reference blocks. | OK. |
| CONS-004 | Detailed design `CONS-004`; detailed appendix `CONS-004`. | Delete/disable referenced constraint; high-risk confirm. | 删除/停用合同约束 | enabled -> disabled. | Required | Disabled; physical delete blocked. | OK, but overlay not schema-driven. |
| CONS-011 | PRD `CONS-011`; detailed appendix `CONS-011`. | View constraint check result. | 查看约束检查结果 | No state change. | Optional | Constraint trace shown; no trace message. | OK. |
