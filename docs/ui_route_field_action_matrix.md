...
| `/allocation/simulation` | `NAV_ALLOC_SIMULATION` | `ALLOC` | 收益分配模拟 | `allocation_scenario`, `allocation_priority_item`, `allocation_result`, `constraint_apply_trace` | total_revenue, priority_allocation_amount, data_provider_revenue_pool, allocation_mode, party_name, raw_weight, normalized_weight, pre_constraint_amount, post_constraint_amount, adjustment_reason, scenario_status | allocation_id, item_id, result_id, trace_id, weight_task_id, source_snapshot_id, checksum, rounding_note | `ALLOC-003`, `ALLOC-005`, `ALLOC-007`, `ALLOC-011`, `ALLOC-013`, `ALLOC-014`, `ALLOC-015`, `ALLOC-016` | UPDATE, CALCULATE, CONFIRM, EXPORT, VIEW | Save revenue/priority/mode writes scenario inputs; execute simulation writes scenario/result/constraint trace; lock confirms scenario; export writes report/export files. |
