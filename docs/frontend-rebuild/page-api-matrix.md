...
| 收益分配模拟 | `/allocation/simulation` | `/allocation/simulation` | `PUT /allocation/simulation/revenue-pool`, `PUT /allocation/simulation/priority-items`, `PUT /allocation/simulation/mode`, `POST /allocation/simulation/run`, `GET /allocation-scenarios/{id}/results`, `POST /allocation/simulation/{id}/lock`, `POST /allocation/simulation/{id}/export` | `allocation_id`, `summary.total_revenue`, `summary.total_contract_priority_amount`, `summary.data_provider_revenue_pool`, `contract_priority_allocations[]`, `data_provider_allocations[]`, `subject_track`, `party_name`, `raw_weight`, `normalized_weight`, `pre_constraint_amount`, `post_constraint_amount`, `constraint_adjustment_reason`, trace rows, `report_id`, `checksum` | `ALLOC-003`, `ALLOC-005`, `ALLOC-007`, `ALLOC-011`, `ALLOC-013`, `ALLOC-014`, `ALLOC-015`, `ALLOC-016` | 需 MD-DShap 权重完成；总收益、非数据源合同优先分配和上限由后端校验；已确认/已导出只读；无 copy 接口时不显示 `ALLOC-014` | 收益流向必须展示“总收益 -\> 非数据源合同优先及上限 -\> 数据源主体收益池 -\> MD-DShap 权重分配”，金额和约束前后对比必须来自后端 DTO。 |
...
| 分配 flow/compare DTO | 总收益 -> 非数据源合同优先及上限 -> 数据源主体收益池 -> 数据源主体分配流向，约束前后差额。 |
