...
| 质量评估管理 | `/measure/quality` | `/metering/quality` | `GET /metering/quality/weights`, `PUT /metering/quality/weights`, `POST /metering/quality/evaluate`, `GET /quality-assessments/latest`, `GET /quality-assessments/{id}/details` | `assessment_id`, `quality_score`, `quality_level`, `quality_factor`, `dimension_scores`, `evidence_summary`, `details[].dimension_name`, `details[].weight`, `details[].score`, `details[].evidence`, `version_no`, snapshot ids | `QUAL-002`, `QUAL-003`, `QUAL-006`, `QUAL-009` | 需有效数据包和资源主体关系；重评生成新版本；权重保存必须后端校验合计 | 质量总分卡、维度图、二级指标表来自 assessment/detail DTO。 |
...
| 收益分配模拟 | `/allocation/simulation` | `/allocation/simulation` | `PUT /allocation/simulation/revenue-pool`, `PUT /allocation/simulation/priority-items`, `PUT /allocation/simulation/mode`, `POST /allocation/simulation/run`, `GET /allocation-scenarios/{id}/results`, `POST /allocation/simulation/{id}/lock`, `POST /allocation/simulation/{id}/export` | `allocation_id`, `total_revenue`, `priority_allocation_amount`, `data_provider_revenue_pool`, `allocation_mode`, `party_name`, `raw_weight`, `normalized_weight`, `pre_constraint_amount`, `post_constraint_amount`, `constraint_adjustment_reason`, trace rows, `report_id`, `checksum` | `ALLOC-003`, `ALLOC-005`, `ALLOC-007`, `ALLOC-011`, `ALLOC-013`, `ALLOC-014`, `ALLOC-015`, `ALLOC-016` | 需 MD-DShap 权重完成；总收益/优先分配由后端校验；已确认/已导出只读；无 copy 接口时不显示 `ALLOC-014` | 收益流向和约束前后对比必须来自 allocation result + constraint trace/chart DTO。 |
| 合同约束管理 | `/allocation/constraints` | `/allocation/constraints` | `GET /allocation/constraints`, `POST /allocation/constraints`, `PATCH /allocation/constraints/{id}`, `PATCH /allocation/constraints/{id}/status` | `constraint_id`, `party_id`, `party_name`, `constraint_name`, `constraint_type`, `value_type`, `constraint_value`, `priority`, `status`, `version_no`, `updated_at` | `CONS-002`, `CONS-003`, `CONS-004`, `CONS-011` | 金额非负、比例 0-1、优先级由后端校验；已被报告引用只能停用 | 约束类型分布、命中/未命中状态来自 constraint/apply trace DTO。 |
| 报告生成与导出 | `/reports` | `/reports` | `GET /reports`, `GET /reports/preview`, `POST /reports/markdown`, `POST /reports/csv`, `POST /reports/json`, `POST /reports/audit-log`, `POST /reports/md-dshap-audit` | `report_id`, `report_type`, `file_name`, `file_format`, `checksum`, `created_at`, `export_file_ids`, `simulation_disclaimer`, `field_scope` | `REP-001`, `REP-002`, `REP-003`, `REP-004`, `REP-005`, `REP-006`, `REP-009` | PDF `REP-003` P1 禁用；报告导出需已完成收益分配；确认书无后端专用接口时禁用 `REP-009` | 导出文件清单、report_id、checksum、生成时间来自 report/export DTO。 |
...
| 用户与权限管理（P1） | `/system/users` | `/system/users` | 无 P0 用户/RBAC API | P1 说明文本，不读写生产用户 | `USER-001`, `USER-002`, `USER-007`, `USER-008`, `USER-009` | P1 全部禁用；不创建真实用户、不登录、不重置密码 | 只读说明，不做 P1 图表。 |
...
| `ALLOC-003/005/007` | 已有 revenue/priority/mode draft | 当前服务未处理 | 接入后端草稿保存。 |
...
| `USER-*` | P1 | 说明页 | 保持 P1 禁用。 |
| `AUD-002/006/007` | 已有 audit logs/detail/export | 查询/导出部分接入 | 详情改用 `/audit-logs/{id}`。 |

## 字段映射注意事项

- 金额字段：`total_revenue`, `priority_allocation_amount`, `data_provider_revenue_pool`, `pre_constraint_amount`, `post_constraint_amount`, `metering_amount` 只能来自后端。
- 权重字段：`normalized_weight`, `participant_weight`, `weight_diff` 只能来自后端，前端只做 6 位小数格式化。
- 质量字段：`quality_score`, `quality_level`, `quality_factor`, `dimension_scores`, detail score/weight 只能来自后端。
...
| 分配 flow/compare DTO | 总收益 -> 优先分配 -> 数据源收益池 -> 参与方分配流向，约束前后差额。 |
